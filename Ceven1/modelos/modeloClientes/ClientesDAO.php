...
class ClientesDAO extends ConBdMySql /* implements InterfaceCRUD */ {
   ...
   private $cantidadTotalRegistros;
    ...
    public function seleccionarTodos() {
        $planConsulta = "select c.cliDocumento,c.cliNombre,c.cliApellido1,c.cliApellido2,c.cliFechaNacimiento,c.cliGenero,c.cliEstado ";
        $planConsulta.= "from clientes c ";
        
        $registrosClientes = $this->conexion->prepare($planConsulta); //Se envia la consulta
        $registrosClientes->execute(); //Ejecución de la consulta
        $listadoRegistrosClientes = array();
        
        while($registro = $registrosClientes->fetch(PDO::FETCH_OBJ)) {
            $listadoRegistrosClientes[] = $registro;
        }
        
        $this->cierreBd();
        return $listadoRegistrosClientes;
        echo "$planConsulta";
    }
    public function consultaPaginada(ClientesVO $consultarClientes = NULL, $limit = NULL, $pagInicio = NULL) {
